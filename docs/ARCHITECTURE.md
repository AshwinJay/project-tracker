...
- **File menu**: "File" dropdown in the toolbar with seven actions:
  ...
  - **Save** — writes directly to the stored `FileSystemFileHandle` if one exists (no re-download); otherwise falls back to a `URL.createObjectURL` download. After a download-based Open, Save always downloads. If there are unsnapshotted changes (content changed since the last snapshot), clicking Save first shows a "Save without snapshot?" confirm modal with three options: **Snapshot & Save** (opens the snapshot label modal; after confirming, saves automatically), **Save without Snapshot**, or **Cancel**.
  - **Save As** — always calls `showSaveFilePicker` to choose a new location (Chrome/Edge), or downloads if the API is unavailable. Updates `fileHandleRef` and stores the filename in localStorage (`project-tracker-last-file`). Same snapshot prompt as Save.
...
- **Unsaved changes indicator**: an amber `●` dot appears on the **Save** menu item and the filename label whenever `isDirty` is true — i.e. current state differs from `lastSavedJson` (the serialised state at the last file open or save). `isDirty` is false when no file is associated (`fileName` is null). Any change to `project`, `scopes`, `risks`, `changes`, or `snapshots` can make the file dirty. The dot disappears as soon as a successful Save or Save As completes. Two derived values underpin this: `isDirty` (drives the dot) and `hasUnsnapshottedChanges` (gates the snapshot prompt on Save — separate because taking a snapshot sets `hasUnsnapshottedChanges = false` while `isDirty` stays true until the file is written).
- **Filename display**: when a file is opened via the FSA API (giving a writable handle), the filename is shown next to the File button in the toolbar — amber when `isDirty`, muted otherwise.
...
- **Modals**: Overlay with backdrop-click-to-close. Used for project settings, add/edit scope, add risk, add change, snapshot label, snapshot restore (confirm and failure/partial), the New/Demo confirmation dialogs, and the "Save without snapshot?" confirm dialog.
